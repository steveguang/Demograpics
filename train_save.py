...
def process_tbn(X_train, y_train, X_test, y_test):
    ...
    if processType == ProcessType.mlp:
        ...
        lstm_model = LstmAttention(batch_size, hidden_size, embedding_length, l_out)
        model = MulLstmAttentionEnsemble(l_out, int(l_out/2), D_out, [lstm_sub_model], lstm_model, bin_label)
...
def eval_model(*args):
    model = args[0]
    #model = torch.load("saved_model/model")
    model.eval()
    if processType == ProcessType.mlp:
        ...
        y_hat_test = model(None, test_names_idx, test_names_len)
    elif processType == ProcessType.tbn_c_name_att:
        model, numerical_test, test_names_idx, embedding_test, test_names_len, y_test = args
        y_hat_test = model(None, embedding_test, [(test_names_idx, test_names_len)])
    else:
        ...
        for i in range(0, len(y_test), batch_size):
            ...
            with torch.no_grad():
                #print (model(numerical_test[i:i+batch_size], embedding_test[i:i+batch_size], [(batch_emoji_idx, None) ]))
                y_hat_test_batch = model(numerical_test[i:i+batch_size], embedding_test[i:i+batch_size], [(batch_emoji_idx, None) ])
                print (y_hat_test_batch.shape)
                y_hat_test += y_hat_test_batch
...
if sourceType == SourceType.race:
    ...
    filename = "transdb_bert-uncased/mergedAllModify.csv"
