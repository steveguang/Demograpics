...
from helper_functions import SeqAttention, CNN, EnsembleModelsAttention
from helper_functions import to_cuda, to_float_cuda, to_self_cuda, under_sample, get_remain_handles, map_handle_gt
from text_processing import load_pretrained_vectors, process_tweet, read_word2idx
    ...
    for epoch in range(epochs):
        ...
        while idx < len(train_names):
            ...
            for i in range(len(batch_train_names)):
                ...
                emoji_idx = process_tweet(handle, dates, longest_emoji_len, word2idx, fix_seq_len)[::-1]
            ...
            y_pred = model(fix_seq_len, None, X_batch_train, seq_lens, [(X_batch_train_emoji,None)] )
            loss = loss_fn(y_pred, y_batch_train)
...
def eval_model(model, test_names):
    ...
    while idx < len(test_names):
        ...
        y_hat_test = model(fix_seq_len, None, X_batch_test, seq_lens,  [(X_batch_test_emoji,None)])
        #print (y_hat_test)
...
def get_fileinfo_from_index(index):
    info = []
    for idx in index:
        info.append(index_to_file_label[idx])
    return info

def get_tweet_embeddings(path, index):
    count = 0
    index = index.tolist()
    index = random.sample(index, len(index))
    X, y = [], []
    for filename, val in get_fileinfo_from_index(index):
        tweet_emb = []
        handle = filename[:-4].lower()
        ...
        for line in f:
            ...
            vals = info[1:]
            tweet_emb.append([float(val) for val in vals])
            if len(tweet_emb) == fix_seq_len:
                break
        f.close()
        X.append(tweet_emb)
        y.append(val)
...
for train_index, test_index in skf.split(index, labels):
    ...
    model = EnsembleModelsAttention(D_in*2, int(D_in*2//2/2), D_out, int(D_in/2), [emoji_cnn_model], bin_label)
