...
from emoji_hashtag_seg2 import get_handle2idx_embeddings, divide_emojis, get_batch_emojis, CNN_NLP
...
def process_tbn(X_train, y_train, X_test, y_test):
    ...
    #y_test = torch.where(y_test>1, 1, 0)
    #y_train = torch.where(y_train>1, 1, 0)
    ...
    if processType == ProcessType.mlp:
        ...
        to_cuda(model)
        train_model(model, embedding_test, y_test, embedding_train, y_train)
        ...
        #print (numerical_train.shape[1]+l_out, int(numerical_train.shape[1]+l_out/2), D_out)
        to_cuda(model)
        train_model(model, numerical_test, embedding_test, y_test, numerical_train, embedding_train, y_train)
...
def train_model(*args):
    ...
    for t in range(epochs):
        for i in range(0, len(y_train), batch_size):
            if processType == ProcessType.mlp:
                model, embedding_test, y_test, embedding_train, y_train = args
                y_pred = model(embedding_train[i:i+batch_size])
                temp.append(eval_model(model, embedding_test, y_test))
            elif processType == ProcessType.name_c_tbn:
                ...
                model, numerical_test, embedding_test, y_test, numerical_train, embedding_train, y_train = args
                y_pred = model(numerical_train[i:i+batch_size], embedding_train[i:i+batch_size])
                temp.append(eval_model(model, numerical_test, embedding_test, y_test))
            elif processType == ProcessType.name:
                ...
                y_pred = model(numerical_train[i:i+batch_size], embedding_train[i:i+batch_size])
            #print (y_pred.shape)
            #print (y_train[i:i+batch_size].shape)
            #print (1/0)
...
processType = ProcessType.tbnn_att
