...
def process_tbn(X_train, y_train, X_test, y_test):
    ...
    y_train = to_float_cuda(y_train) #to_float_cuda(y_train.reshape(-1, 1)) if sourceType != SourceType.age else to_self_cuda(y_train)
    ...
    y_test = to_self_cuda(y_test) #to_float_cuda(y_test.reshape(-1, 1)) if sourceType == SourceType.age else to_self_cuda(y_test)
    ...
    if processType == ProcessType.mlp:
        ...
        index = y_train[:]>1
        embedding_train_greater, numerical_train_greater = embedding_train[index], numerical_train[index]
        y_train_greater = y_train[index]
        y_train_greater = transfer_y(y_train_greater, True, 2, 1, 0)
        counter = [0, 0]
        lstm_model1 = LstmAttention(batch_size, hidden_size, embedding_length, l_out)
        ...
        train_model(model1, numerical_train_greater, embedding_train_greater, y_train_greater)
        print ("--------1")

        index = y_train[:]<=1
        embedding_train_smaller, numerical_train_smaller = embedding_train[index], numerical_train[index]
        y_train_smaller = y_train[index]
        lstm_model2 = LstmAttention(batch_size, hidden_size, embedding_length, l_out)
        ...
        to_cuda(lstm_model2)
        to_cuda(model2)
        train_model(model2, numerical_train_smaller, embedding_train_smaller, y_train_smaller)
        print ("--------2")
        ...
        auc = han_eval_model([model1, model2, model], numerical_test, embedding_test, y_test)
...
def han_eval_model(*args):
    model_greater, model_smaller, model = args[0]
    model_greater.eval()
    model_smaller.eval()
    model.eval()
    if processType == ProcessType.mlp:
        ...
        #y_hat_test = np.argmax(y_hat_test.cpu().detach().numpy(), 1)
        #print (y_hat_test.tolist())
        y_hat_test = np.where(y_hat_test.cpu().detach().numpy()<0.5, 0, 1)
        #print (y_hat_test)
        #print ("binary first")
        y_hat_test_temp = y_hat_test
        y_test_temp = torch.squeeze(y_test).cpu().detach().numpy()
        y_test_temp = transfer_y(y_test_temp, True, 1, 1, 0, True)
        #print (y_hat_test_temp.tolist())
        #print (y_test_temp.tolist())
        print (f1_score(y_test_temp, y_hat_test_temp, average='macro'))

        y_hat_test = y_hat_test.ravel()
        index = y_hat_test[:]>0
        embedding_test_greater = embedding_test[index]
        y_hat_test_greater = model_greater(embedding_test_greater)
        y_hat_test_greater = np.where(y_hat_test_greater.cpu().detach().numpy()<0.5, 0, 1)
        #y_hat_test_greater = np.argmax(y_hat_test_greater.cpu().detach().numpy(), 1)
        #print (y_hat_test_greater)
        y_hat_test_greater = transfer_y(y_hat_test_greater, True, 0, 3, 2, True)
        y_test_greater = y_test[index]
        #print (y_hat_test_greater)
        #print (y_test_greater)

        index = y_hat_test[:]==0
        embedding_test_smaller = embedding_test[index]
        y_hat_test_smaller = model_smaller(embedding_test_smaller)
        #y_hat_test_smaller = np.argmax(y_hat_test_smaller.cpu().detach().numpy(), 1)
        y_hat_test_smaller = np.where(y_hat_test_smaller.cpu().detach().numpy()<0.5, 0, 1)
        y_test_smaller = y_test[index]
        #print (y_hat_test_smaller)

        y_test_permute = torch.cat((y_test_greater, y_test_smaller), 0)
        y_hat_test_permute = np.concatenate((y_hat_test_greater, y_hat_test_smaller), 0)

        y_test_permute = torch.squeeze(y_test_permute).cpu().detach().numpy()
        f1 = f1_score(y_test_permute, y_hat_test_permute, average='macro')
    else:
        models, numerical_test, embedding_test, y_test = args
        ...
        y_hat_test = y_hat_test.ravel()
        index = y_hat_test[:]>0
        embedding_test_greater, numerical_test_greater = embedding_test[index], numerical_test[index]
        y_hat_test_greater = model_greater(numerical_test_greater, embedding_test_greater)
        y_hat_test_greater = np.where(y_hat_test_greater.cpu().detach().numpy()<0.5, 0, 1)
        #y_hat_test_greater = np.argmax(y_hat_test_greater.cpu().detach().numpy(), 1)
        #print (y_hat_test_greater)
        y_hat_test_greater = transfer_y(y_hat_test_greater, True, 0, 3, 2, True)
        ...
        index = y_hat_test[:]==0
        embedding_test_smaller, numerical_test_smaller = embedding_test[index], numerical_test[index]
        y_hat_test_smaller = model_smaller(numerical_test_smaller, embedding_test_smaller)
        #y_hat_test_smaller = np.argmax(y_hat_test_smaller.cpu().detach().numpy(), 1)
        y_hat_test_smaller = np.where(y_hat_test_smaller.cpu().detach().numpy()<0.5, 0, 1)
        y_test_smaller = y_test[index]
    ...
    print (f1)
    print ("------------")
    #print (y_test_permute.tolist())
    #print (y_hat_test_permute.tolist())
    print (classification_report(y_test_permute, y_hat_test_permute))
    return f1
...
def eval_model(*args):
    ...
    y_hat_test_class = np.where(y_hat_test.cpu().detach().numpy()<0.5, 0, 1) #np.argmax(y_hat_test.cpu().detach().numpy(), axis=1) if sourceType == SourceType.age else np.where(y_hat_test.cpu().detach().numpy()<0.5, 0, 1)
...
def transfer_y(y, is_greater, mark, v1, v2, use_np=False):
    if use_np:
        return np.where(y>mark, v1*1.0, v2*1.0) if is_greater else np.where(y<=mark, v1*1.0, v2*1.0)
    return torch.where(y>mark, v1*1.0, v2*1.0) if is_greater else torch.where(y<=mark, v1*1.0, v2*1.0)
...
valid = False
data_type = DataType.tbn
batch_size = 14315
sourceType = SourceType.age
...
bins = [0, 35, 55, 120]
bins = [0, 30, 40, 50, 120]
D_out = 1 #len(bins)-1 if sourceType == SourceType.age else 1
if sourceType == SourceType.race:
    ...
    #filename = "transdb_bert-uncased-nli/merge_everything.py"
