...
def process_tbn(X_train, y_train, X_test, y_test):
    ...
    if processType == ProcessType.mlp:
        ...
        auc = han_eval_model([model2, model], embedding_test, y_test)
...
def train_model(*args):
    ...
    #loss_fn = F.nll_loss if sourceType == SourceType.age else nn.BCELoss()
...
def han_eval_model(*args):
    ...
    if True:
        ...
        index = y_hat_test[:]>0
        y_hat_test_greater = y_hat_test[index]
        y_hat_test_greater = transfer_y(y_hat_test_greater, True, 0, 2, -1, True)
...
def transfer_y(y, is_greater, mark, v1, v2, use_np=False):
    if use_np:
        return np.where(y>mark, v1, v2) if is_greater else np.where(y<=mark, v1, v2)
    return torch.where(y>mark, v1, v2) if is_greater else torch.where(y<=mark, v1, v2)
...
bins = [0, 35, 55, 120]
#bins = [0, 30, 40, 50, 120]
D_out = 2 #len(bins)-1 if sourceType == SourceType.age else 1
